# LẬP LỊCH CPU - THUẬT TOÁN ROUND ROBIN
# (Preemptive - chia thời gian theo Time Quantum)
# Công thức:
...
def nhap_tien_trinh():
    """Nhập danh sách tiến trình và Time Quantum từ bàn phím."""
    print("      LẬP LỊCH CPU - THUẬT TOÁN ROUND ROBIN")
    ...
    while True:
        try:
            quantum = int(input("Nhập Time Quantum: "))
            if quantum <= 0:
                print("  [!] Time Quantum phải >= 1. Vui lòng nhập lại.")
                continue
            break
        except ValueError:
            print("  [!] Vui lòng nhập số nguyên.")
...
def tinh_round_robin(tien_trinh, quantum):
    """
    Áp dụng thuật toán Round Robin:
      - Các tiến trình được đưa vào hàng đợi Ready Queue theo Arrival Time.
      - Mỗi tiến trình chạy tối đa bằng Time Quantum.
      - Nếu chưa xong thì quay lại cuối hàng đợi.
    ...
    while da_xong < n:
        # Nếu hàng đợi rỗng và chưa có tiến trình đến, CPU rảnh
        if not ready_queue and i < n and thoi_gian < ds[i]["at"]:
            them_doan_gantt(gantt, "Idle", thoi_gian, ds[i]["at"])
            thoi_gian = ds[i]["at"]

        # Đưa các tiến trình đã đến vào Ready Queue
        while i < n and ds[i]["at"] <= thoi_gian:
            ready_queue.append(ds[i])
            i += 1
        ...
        start = thoi_gian
        thoi_gian_chay = min(quantum, con_lai[pid])
        thoi_gian += thoi_gian_chay
        con_lai[pid] -= thoi_gian_chay
        them_doan_gantt(gantt, pid, start, thoi_gian)

        # Trong lúc tiến trình đang chạy, có thể có tiến trình mới đến
        while i < n and ds[i]["at"] <= thoi_gian:
            ready_queue.append(ds[i])
            i += 1

        # Nếu tiến trình chưa xong, đưa về cuối hàng đợi
        if con_lai[pid] > 0:
    ...
    for label, start, end in gantt:
        if label == "Idle":
            ...
            print(f"  t={start:>3} → {end:>3} : Chạy {label} "
                  f"trong {end - start} đơn vị → còn tính đến khi CT={r['ct']}, "
                  f"TAT={r['tat']}, WT={r['wt']}")
...
if __name__ == "__main__":
    while True:
        tien_trinh, quantum = nhap_tien_trinh()
        ket_qua, gantt = tinh_round_robin(tien_trinh, quantum)
